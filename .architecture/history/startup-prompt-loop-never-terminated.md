...
audit round after `door-retry-loop-spun-on-a-cancelled-token.md`.
...
**Rule:** A loop that re-derives its own work list needs a termination rule that does not
depend on the work changing the state the list is derived from. Either prove the work list
shrinks on each pass, or make the sequence monotone. A test that exercises only the branch
that terminates proves nothing about the loop.
