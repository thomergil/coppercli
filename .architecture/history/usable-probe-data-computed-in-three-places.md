# Usable probe data was computed in three places
...
applies the constructor's geometry checks (`ValidateLoadedGridGeometry`) — finite ordered extents, at
...
`validate-loaded-grid`, `one-field-per-fact`,
`derived-artifact-records-its-context`, `no-side-effect-on-get`, `releaseasync-returns-controller-to-idle`,
`fail-safe-on-uncertainty`, `tests-detect-plausible-defects`; interface `probe data lifecycle`
v3.
...
- Making `HandleMillStopAsync` call `ReleaseAsync()` unconditionally. The run's own teardown
  had exceeded the stop timeout and was still stopping the machine and lifting the tool, and
...
**Rule:** Define usable probe data once in `AppState.CurrentProbeGrid` and use it for checks and displays. Validate a loaded grid by the same rules used when constructing one.
