...
- An unrestored depth adjustment is reported. coppercli writes the adjustment into the work
  origin and takes it back out when the run ends, but a machine that refuses the write -
  alarmed at the end of a run, for example - left it in place silently, so every later job
  cut that much deeper or shallower, whichever way you had set it, and the amount was
  forgotten.
...
- The file screen offers to apply a height map that exists only in the saved copy. It said
  ...
  applied.
- A take-over that the server refuses reports why, rather than reloading the page into the
  same dialog.
...
- A single Z probe in the browser can be stopped. It reports what went wrong rather than
...
- The line counter on the mill screen has one source, so it no longer flickers between two
  values arriving at different rates. A reconnect no longer shows an internal phase name.
