...
**Controller FSM documentation:** The valid state transitions are the table at the top of `coppercli.Core/Controllers/ControllerBase.cs`; each controller's phases are documented on its own `*Phase` enum. For probe data lifecycle (autosave, recovery, save states), see the detailed documentation at the top of `coppercli.Core/Controllers/ProbeController.cs`.
