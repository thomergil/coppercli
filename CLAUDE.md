...
**Prompts:** Use `MenuHelpers.AskString()`, `MenuHelpers.AskDouble()`, `MenuHelpers.Confirm()`, `MenuHelpers.ConfirmOrQuit()`, or `MenuHelpers.ConfirmOrExit()` at startup. Never use raw `Console.ReadLine()`.
...
- **`MenuHelpers.cs`** - Menus, prompts, confirmations, validation (`GetMachineBlocker` for the machine, `GetProbeDisabledReason` and `CheckMillCanStart` for the job), `ConfirmOrExit` (a yes/no at startup, where quitting ends the program), and `WaitForDoorClear` (`MachineWait.ClearDoorHoldAsync` for a screen with no run behind it, asked through a `DoorPrompts`: `Overlay` on a full-screen view, `ScrollingConsole` at startup)
